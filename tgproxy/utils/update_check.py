"""
Check for a new version via the GitHub Releases API

Request rate limit: no more than once per hour per machine (cache in the
app data directory). If-None-Match (ETag) is supported for 304 responses.
"""
...
# No more than one full API request per hour (304 with the same ETag not counted).
...
def _version_gt(a: str, b: str) -> bool:
    """True if version a is newer than b (simple per-segment comparison)."""
...
def fetch_latest_release(
    ...
    """
    GET releases/latest. Returns (data or None on 304, etag or None, HTTP code).
    """
    headers = {
...
def run_check(current_version: str) -> None:
    """Fetches the latest release and updates the internal state."""
    global _state
    _state["checked"] = True
    _state["error"] = None
    ...
    if last_attempt and (now - last_attempt) < _MIN_FETCH_INTERVAL_SEC:
        ...
        _state["error"] = (
            err if err else "Update check deferred (request interval)."
        )
        _state["has_update"] = False
        _state["ahead_of_release"] = False
        _state["latest"] = None
    ...
    try:
        ...
        if isinstance(e, HTTPError) and e.code == 403:
            msg = (
                "GitHub API returned 403 (rate limit or access). Try again later."
            )
...
def get_status() -> Dict[str, Any]:
    """State snapshot after run_check (for display in settings)."""
    return dict(_state)
